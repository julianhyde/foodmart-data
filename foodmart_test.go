...
func TestLicenseHeader(t *testing.T) {
	...
	// A walk that matched nothing would pass silently. Only Go files
	// are required: not every checkout holds the Rust crate, but this
	// is a Go test, so foodmart_test.go itself is always found.
	if counts[".go"] == 0 {
		t.Error("found no .go files to check")
	}
